                    ...
                    if (is_array($history) && count($history) > 1) {
                        ...
                        if ($item['Ident'] === 'Wind_Direction') {
                            ...
                            if ($speedID && AC_GetLoggingStatus($archiveID, $speedID)) {
                                ...
                                $dataString = "{ type: 'area', data: [" . implode(',', $speedPoints) . "], color: '$chartColor', fillOpacity: 0.3, zIndex: 1 }, { type: 'windbarb', data: [" . implode(',', $barbPoints) . "], color: '$fontColor', zIndex: 2, vectorLength: 55, yOffset: -10 }";
                            }
                        } else {
                            foreach (array_reverse($history) as $row) {
